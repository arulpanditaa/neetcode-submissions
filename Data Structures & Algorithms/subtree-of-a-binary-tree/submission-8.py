...
class Solution:   
    ...
    def z_func(self, s: str) -> list: 
        ...
        for i in range(1, n):
            if i <= r:
                z[i] = min(r - i + 1, z[i - l])
# We take the smaller of our past pattern memory (z[i-l]) and our verified window's remaining space (r-i+1) to get the maximum safe headstart before testing uncharted territory. 
